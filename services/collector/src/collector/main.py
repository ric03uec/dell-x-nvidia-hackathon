...
def run(source: str, ingestion_url: str) -> int:
    posted = dropped = 0
    for line in follow(source):
        ...
        if post_event(ingestion_url, record):
            ...
            dropped += 1
    print(f"collector: source ended. posted={posted} dropped={dropped}", file=sys.stderr)
...
def main(argv: list[str] | None = None) -> int:
    ...
    return run(args.source, args.ingestion_url)
